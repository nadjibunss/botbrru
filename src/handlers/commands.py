...
async def receive_cookie(
    ...
    username = (
        f" untuk <b>{html.escape(result.account_username)}</b>"
        if result.account_username
        else ""
    )

    await send_message(
        chat_id,
        f"✅ Cookie sesi berhasil disimpan{username}.\n\n"
        "Jika sesi berakhir, jalankan /setcredentials untuk memperbaruinya.",
    )
...
async def command_status(
    ...
    text = (
        f"{'🟢' if active else '🔴'} <b>Status:</b> "
        f"{'Aktif' if active else 'Tidak aktif'}\n\n"
        f"<b>Akun:</b> {account}\n"
